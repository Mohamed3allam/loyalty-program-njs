...
const userSchema = new Schema({
    ...
    profile_pic_url:{
        type:String,
        required: false,
        default: "https://reward-program-4design.s3.amazonaws.com/profile-pics/default/default.png"
    },
    user_created_at:{
        type:String,
        required: true
...
userSchema.statics.login = async function( email, password ) {
    ...
    // validation
    if ( !email || !password ) {
        // f = failures[remoteIp] = failures[remoteIp] || {count: 0, nextTry: new Date()};
        // ++f.count;
        // f.nextTry.setTime(Date.now() + 2000 * f.count); // Wait another two seconds for every failed 
        throw Error("All fields must be filled")
    }
